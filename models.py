...
class BooksModel:
    ...
    def get_all(self):
        ...
        cursor = self.connection.cursor()
        cursor.execute("SELECT model, price, book_id FROM books")
        rows = cursor.fetchall()
        return rows
    ...
    def get_by_price(self, start_price, end_price):
        ...
        cursor = self.connection.cursor()
        cursor.execute("SELECT model, price, book_id FROM books WHERE price >= ? AND price <= ?", (str(start_price), str(end_price)))
        row = cursor.fetchall()
        return row
    ...
    def get_by_dealer(self, dealer_id):
        ...
        cursor = self.connection.cursor()
        cursor.execute("SELECT model, price, book_id FROM books WHERE dealer = ?", (str(dealer_id)))
        row = cursor.fetchall()
        return row
